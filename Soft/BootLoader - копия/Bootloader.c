...
__task void main(void)
{
  ...
  if( target.mcuStatus ) 
  {
    ...
    else 
    {
      ...
      Application = (funcptr)(*(uint32_t*)((*((uint32_t*)(config->dataAdress + 4)))+4)); // переходим на основную прошивку
      /* Initialize user application's Stack Pointer */
      __set_MSP((*(uint32_t*)((*((uint32_t*)(config->dataAdress + 4)))))); // переходим на основную прошивку
      Application(); // переходим на основную прошивку
    }
  } 
  else 
  {
    ...
    if( RxTail != RxHead ) 
    {
      ...
      if(CHANNEL == CH_BOOTLOADER) 
      {
        ...
        case TM_CMD_ACK_6:
          ...
          case CMD_PROGEEPROM:
            /* Программирование EEPROM не поддерживается! */
            temp = ERR_CMD_FAIL;
            goto  err_prog;
